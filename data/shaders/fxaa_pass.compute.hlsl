...
void cs_main(uint3 dt_id : SV_DispatchThreadId)
{
	...
	//
	const float luma_ne = g_tex_tone_mapping.Load(uint3(dt_id.xy, 0), uint2(1, -1)).a;
	const float luma_sw = g_tex_tone_mapping.Load(uint3(dt_id.xy, 0), uint2(-1, 1)).a;
	// edge horz
	...
	bool reached_end_1 = (abs(luma_edge - luma_end_1) >= gradient_scaled);
